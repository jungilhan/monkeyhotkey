...
progressDialog::progressDialog(QWidget *parent)
    ...
    , m_okButton(new QPushButton(trUtf8("확인"), this))
    , m_cancelButton(new QPushButton(trUtf8("취소"), this))
    , m_timer(0)
    , m_autoCloseMsec(-1)
    , m_currentMsec(-1)
...
void progressDialog::init()
{
    ...
    m_okButton->connect(m_okButton, SIGNAL(clicked()), this, SLOT(accept()));

    m_cancelButton->setGeometry(m_okButton->x() + m_okButton->width() + 10, 75, 75, 25);
    m_cancelButton->connect(m_cancelButton, SIGNAL(clicked()), this, SLOT(reject()));
}
